...
router.post(
  "/download-all",
  express.json(),
  narrationController.downloadAllAudio,
);

// Prepare aligned narration timing plan for direct clip playback
router.post(
  "/preview-aligned",
  express.json({ limit: "50mb" }),
  narrationController.previewAlignedAudio,
);

// Generate aligned narration metadata for local streaming playback
router.post(
  "/generate-aligned",
  express.json({ limit: "50mb" }),
  narrationController.generateAlignedAudio,
);
...
router.post(
  ...
  narrationController.recordReference,
);
...
router.post(
  "/upload-reference",
  upload.single("file"),
  narrationController.uploadReference,
);
...
router.post("/cleanup-old-directories", express.json(), (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Old subtitle directories cleaned up successfully",
    });
  } catch (error) {
    console.error("Error cleaning up old subtitle directories:", error);
    res.status(500).json({
      success: false,
      error: "Failed to clean up old subtitle directories",
    });
  }
});
...
router.post(
  ...
  narrationController.saveGeminiAudio,
);
...
router.post(
  "/save-f5tts-audio",
  express.json({ limit: "10mb" }),
  narrationController.saveF5TTSAudio,
);

// Save Chatterbox audio data to disk
router.post(
  "/save-chatterbox-audio",
  express.json({ limit: "10mb" }),
  narrationController.saveChatterboxAudio,
);
...
router.post(
  "/modify-audio-speed",
  express.json(),
  narrationController.modifyAudioSpeed,
);

// Batch modify audio speed for multiple files
router.post(
  "/batch-modify-audio-speed",
  express.json(),
  narrationController.batchModifyAudioSpeed,
);

// Combined modify trim + speed (single)
router.post(
  "/modify-audio-trim-speed-combined",
  express.json(),
  narrationController.modifyAudioTrimAndSpeedCombined,
);

// Batch modify audio trim for multiple files
router.post(
  "/batch-modify-audio-trim",
  express.json(),
  narrationController.batchModifyAudioTrim,
);

// Batch combined trim + speed for multiple files
router.post(
  "/batch-modify-audio-trim-speed-combined",
  express.json(),
  narrationController.batchModifyAudioTrimAndSpeedCombined,
);
...
router.post(
  "/get-audio-duration",
  express.json(),
  narrationController.getAudioDuration,
);
...
router.post(
  "/batch-get-audio-durations",
  express.json(),
  narrationController.batchGetAudioDurations,
);
...
router.post(
  ...
  express.json(),
  narrationController.uploadExampleAudio,
);
...
router.use("/", async (req, res, next) => {
  ...
  if (
    ...
    req.url === "/clear-output" ||
    req.url === "/save-gemini-audio" ||
    req.url === "/save-f5tts-audio" ||
    req.url === "/save-chatterbox-audio" ||
    req.url === "/modify-audio-speed" ||
    req.url === "/batch-modify-audio-speed" ||
    req.url === "/modify-audio-trim" ||
    req.url === "/modify-audio-trim-speed-combined" ||
    req.url === "/batch-modify-audio-trim" ||
    req.url === "/batch-modify-audio-trim-speed-combined" ||
    req.url === "/get-audio-duration" ||
  ...
  try {
    ...
    if (req.method === "GET" && req.url.startsWith("/audio/")) {
      try {
        const audioData = await narrationServiceClient.fetchAudioFile(
          req.url.replace("/audio/", ""),
        );
