/**
 * Files-API + offsets path for parallel Gemini streaming.
 *
 * Splits a segment into sub-segments and streams each via the already-uploaded
 * Files API URI, using video_metadata start/end offsets per sub-segment.
 */
...
export const coordinateParallelStreaming = async (
        ...
        return new Promise((resolve, reject) => {
            ...
            streamGeminiContent(
                ...
                async (error) => {
                    ...
                    const shouldRetry = (is503Error(error) || is429Error(error)) && retryCount < maxRetries;

                    if (shouldRetry) {
                        const errorType = is429Error(error) ? '429 (rate limit)' : '503 (overload)';
                        // Progressive retry delays: 5, 10, 15, 20, 25 seconds
                        const retryDelaySeconds = getRetryDelaySeconds(retryCount);
    ...
    // Create a task creator for each segment (don't execute immediately)
     const taskCreators = subSegments.map((subSegment, index) => {
         return () => streamWithRetry(subSegment, index, 0, MAX_RETRIES); // Increased from 3 to 5 retry attempts
     });
