...
export class Morph {
     ...
     * @private
     * @type {Array<{first: Cubic, second: Cubic}>}
     */
    #morphMatch;
     ...
     *
     * @param {Float32Array} [bounds=new Float32Array(4)] - A buffer to hold the results. If not
     *   supplied, a temporary buffer will be created.
     * @param {boolean} [approximate=true] - When true, uses a faster calculation to create the
     *   bounding box based on the min/max values of all anchor and control points that make up the
     *   shape.
     * @returns {Float32Array} The axis-aligned bounding box for this object, where the rectangle's
     *   left, top, right, and bottom values will be stored in entries 0, 1, 2, and 3, in that
     *   order.
     */
     ...
     * @param {Float32Array} [bounds=new Float32Array(4)] - A buffer to hold the results. If not
     *   supplied, a temporary buffer will be created.
     * @returns {Float32Array} The axis-aligned max bounding box for this object, where the
     *   rectangle's left, top, right, and bottom values will be stored in entries 0, 1, 2, and 3,
     *   in that order.
     */
    ...
    asCubics(progress) {
        ...
        if (lastCubic !== null && firstCubic !== null) {
            const closingPoints = new Float32Array([
                lastCubic.anchor0X,
                lastCubic.anchor0Y,
                lastCubic.control0X,
                lastCubic.control0Y,
            ...
            result.push(new Cubic(closingPoints));
        }
        return result;
    }
     ...
     *
     * @param {RoundedPolygon} p1 The starting polygon.
     * @param {RoundedPolygon} p2 The ending polygon.
     * @returns {Array<{first: Cubic, second: Cubic}>} A list of matched cubic pairs.
     * @internal
     */
    static match(p1, p2) {
        ...
        console.log(`🔍 doubleMapper created:`, doubleMapper);
        ...
        if (DEBUG) {
            for (let index = 0; index < bs1.size; index++) {
                debugLog(LOG_TAG, `start ${index}: ${bs1.get(index)}`);
            }
            for (let index = 0; index < bs2.size; index++) {
                debugLog(LOG_TAG, `End ${index}: ${bs2.get(index)}`);
            }
        }
        ...
        while (b1 && b2) {
            ...
            if (DEBUG) debugLog(LOG_TAG, `Match: ${seg1} -> ${seg2}`);
        ...
        if (b1 || b2) {
            throw new Error("Expected both Polygon's Cubics to be fully matched");
        }

        if (DEBUG) {
            // Export as SVG path.
            const showPoint = (p) => `${(p.x * 100).toFixed(2)} ${(p.y * 100).toFixed(2)}`;
            [0, 1].forEach(listIx => {
                const points = ret.map(pair => (listIx === 0 ? pair.first : pair.second));
                const pathData = "M " +
                    showPoint(new Point(points[0].anchor0X, points[0].anchor0Y)) +
                    " " +
                    points.map(it =>
                        "C " +
                        showPoint(new Point(it.control0X, it.control0Y)) +
                        ", " +
